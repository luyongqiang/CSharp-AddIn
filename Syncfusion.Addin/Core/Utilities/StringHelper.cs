...
namespace Syncfusion.Core.Utilities
{
    /// <summary>
    ...
    public static class StringHelper
    {
        ...
        {
            char[] chars = originalString.Trim().ToCharArray();
            StringBuilder sb = new StringBuilder();
            bool bLastWasNewWord = true;
            bool bEncounteredLower = false;
            bool bEncounteredUpper = false;
            for (int counter = 0; counter < chars.Length; counter++)
            {
                ...
                else if (bEncounteredUpper && bEncounteredLower)
                {
                    sb.Append(dummy);
                }
                else
        ...
        {
            StringHelper.ToFile(expression, fileName, Encoding.Default);
        }
        ...
        public static int At(string searchFor, string searchIn, int occurrence)
        {
            ...
            int position = 0;
            int result;
            for (int counter = 1; counter <= occurrence; counter++)
            {
        ...
        {
            long length = (long)sourceString.Length;
            long wordCount = 0L;
        ...
        public static string GetWordNumb(string sourceString, int wordPosition)
        {
            ...
            {
                string[] words = sourceString.Split(new char[]
                {
                    ' '
                });
                result = ((wordPosition <= words.Length) ? words[wordPosition - 1] : string.Empty);
            }
            return result;
        }
        ...
        {
            return StringHelper.GetWordNumb(sourceString, wordPosition);
        }
        ...
        {
            return StringHelper.SubStr(expression, startIndex, length);
        }
        ...
        {
            string result;
            if (startPosition >= expression.Length)
            {
                result = string.Empty;
            }
            else
            {
                result = ((length + startPosition - 1 > expression.Length) ? expression.Substring(startPosition - 1) : expression.Substring(startPosition - 1, length));
            }
            return result;
        }
        ...
        {
            int counter = 0;
            int position = StringHelper.At(searchExpression, expressionSearched);
            if (position > 0 && position < expressionSearched.Length)
            {
                string text = StringHelper.SubStr(expressionSearched, 1, position - 1);
                counter = StringHelper.Occurs("\\r", text) + 1;
            }
            return counter;
        }
        ...
        {
            return path.Substring(0, StringHelper.At("\\", path, StringHelper.Occurs("\\", path)) - 1);
        }
        ...
        {
            return StringHelper.ArrayContainsString(hostArray, searchText, false);
        }
        ...
        {
            return hostArray.Any((string item) => StringHelper.Compare(item, searchText, ignoreCase));
        }
        ...
        {
            return StringHelper.TryGuidParse(value, Guid.Empty);
        }
